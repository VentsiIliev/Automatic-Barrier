...
from shared.repositories.csv_repositories.Constants import ACCESS_GRANTED_FIELDS, EVENT_TYPE, DATE, TIME, REGISTRATION_NUMBER, \
    DIRECTION, OWNER
...
class CSVAccessGrantedRepository(BaseCSVRepository):
    ...
    def get_all(self):
        """Retrieve all access events from the repository."""
        df = self._read_rows()
        return [
            AccessEvent(
                row[EVENT_TYPE],
    ...
    def insert(self, event_type, date, time, registration_number, direction, owner):
        """Insert a new access event into the repository."""
        super().insert(**{
